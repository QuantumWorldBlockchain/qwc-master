...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete qwc database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no qwc datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old qwc datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected qwc 0.7 datadir."
    ;;
  3)
    echo "Detected qwc pre-0.8 datadir."
    ;;
  4)
    echo "Detected qwc 0.8 datadir."
    ;;
esac
