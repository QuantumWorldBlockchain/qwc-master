...
<b><u>Installing Dependeincies for qwc Coin on Your VPS</u></b><br><br>

Before you can run a masternode you have to be able to run the qwc Coin daemon/wallet.  This requires the installation of several dependenicies.  To install these run the following commands on your VPS:<br><br>
...
1. Create an address in your local wallet by:<br>
    ...
    b. Command line: entering command ./qwc-cli getnewaddress<br><br>
The address will be presented back to you, copy it for use in the next steps

2. Send your local wallet 20000 qwc by:<br>  
    ...
    b. Command line: entering command ./qwc-cli sendtoaddress "address from step 1" 20000.  The system will give you the TransID as confirmation <br>


3. Now wait for your transaction to have 15 confirmations so the network has been properly updated.  Do this by:
    a. QT/GUI Wallet: Hovering your mouse over the transaction which will be called "payment to yourself".  Don't worry, it won't show 20000 qwc coin, only the fee. The wallet will pop up a message such as (6 confirmations, will be available after 10).  You are wating on this to reach 15 confirmations.<br><br>
    b. Command line: entering command ./qwc-cli listtransactions.  The system will give an output similar to the below, you are waiting on the confirmations to reach 15.<br>
...
b. Command line: enter the following 2 commands and save the long string the system will give you for use them in steps:<br>
            ./qwc-cli masternode outputs<br>
            ./qwc-cli masternode genkey<br><br> 
...
mkdir ~/.qwccore && nano ~/.qwccore/masternode.conf<br><br>
...
The second file you will create is named qwc.conf and will only be done on the VPS.  Create the file in nano by entering the following command:<br><br>

        nano ~/.qwccore/qwc.conf
...
It is now time to download and install the qwc Coin daemon to run on your VPS/Linux Server.  enter the following commands to download and extract the daemon on your VPS.  If you decide to do this in a different folder other than your home folder you need to kconsider that when starting it alter.  This guide assumes you are putting in in your home folder and running as the root user that most VPS providers give you.  First navigate to the qwc Coin github at https://github.com/qwccoin/qwc/releases and find the release that matches the VPS operating system you arerunning.  This guide is running it on ubuntu 18.04 so substitute in the url that best matching your system:<br><br>

        wget https://github.com/qwccoin/qwc/releases/download/v2.0.0.2/qwc-v2002-ubuntu18-64.zip
        unzip qwc-v2002-ubuntu18-64.zip
        chmod qwcd qwc-cl qwc-tx 755
...
When the "IsSynced" row say true you are ready to start the node up by using command: <br><br>
        ...
        ./qwc-cli masternode start-alias mn
 <br><br>       
...
You can check status of your masternode by using command: <br><br>
        ...
        ./qwc-cli masternode status
<br><br>
If you did everything right you will see "Successfully Started" in the message given

Now its time to set up Sentinel to keep things running smoothly and avoid WATCHDOG_EXPIRED status.  This is not required as your node will still receive paymenrs but still looks better.  Visit https://github.com/qwccoin/sentinel/blob/master/README.md to continue for Linux and https://github.com/qwccoin/sentinel/blob/master/sentinel-for-windows.md for Windows
