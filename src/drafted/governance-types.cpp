...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "qwcNetwork") type = qwcNetwork;
    if(strType == "qwcNetworkVariable") type = qwcNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == qwcNetwork) s = "qwcNetwork";
    if(type == qwcNetworkVariable) s = "qwcNetworkVariable";
