...
const char * const BITCOIN_CONF_FILENAME = "qwc.conf";
const char * const BITCOIN_PID_FILENAME = "qwcd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "qwc" is a composite category enabling all qwc-related debug output
            if(ptrCategory->count(string("qwc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\qwcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\qwcCore
    // Mac: ~/Library/Application Support/qwcCore
    // Unix: ~/.qwccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "qwcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/qwcCore";
#else
    // Unix
    return pathRet / ".qwccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty qwc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override qwc.conf
